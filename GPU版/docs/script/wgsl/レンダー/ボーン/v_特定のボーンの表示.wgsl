...
@group(3) @binding(0) var<storage, read> activeIndexs: array<u32>;
...
fn main(
    ...
    // 頂点データを取得
    let index = activeIndexs[instanceIndex];
    ...
    if (vertexIndex == 0u) {
        // offset = position1 - (normal * size * 10.0) / camera.zoom;
        offset = position1 - (normal * size * length(sub));
