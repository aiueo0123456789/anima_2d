...
const pointData = array<vec4<f32>, 4>(
    vec4<f32>(-0.1, 0.0, 0.0, 1.0), // 左下
    vec4<f32>(0.0, 1.0, 0.0, 0.0), // 左上
    vec4<f32>(0.1, 0.0, 1.0, 1.0), // 右下
    vec4<f32>(0.0, 1.0, 1.0, 0.0), // 右上
);
...
fn main(
    ...
    output.position = vec4f((data.position + rotate2D(point.xy * size * data.scale, data.angle) - camera.position) * camera.zoom * cvsAspect, 0, 1.0);
